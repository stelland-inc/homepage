...
export function DottedBackground({
    ...
    useEffect(() => {
        ...
        // Sized immediately, synchronously — the rAF-scheduled version below
        // is fine for ongoing resize events, but gating the VERY FIRST size
        // behind a rAF callback meant the canvas could stay stuck at its
        // construction-time 300x150 default if that first frame never got
        // a chance to run before something else read the canvas size.
        doResize();
        ...
        // devicePixelRatio changes (e.g. browser zoom) don't always fire a
        // window "resize" event, so watch it explicitly via matchMedia.
        let dprMediaQuery: MediaQueryList | null = null;
        const watchDpr = () => {
            if (dprMediaQuery) dprMediaQuery.removeEventListener("change", onDprChange);
...
/* —— Hero shell —— */
/**
 * Public asset URLs (catalogue + iframe). Use a function — not `` `${A}/file` `` —
 * so preview rewrite does not break src when the admin asset map is incomplete.
 */
function asset(file: string) {
  return `/originkit/hero-26/${file}`;
}

type DotmatrixHeroProps = {
  badge?: string;
  titleLine1?: string;
  titleLine2?: string;
  description?: string;
  ...
  docsLabel?: string;
  joinLabel?: string;
  /** The site already has its own persistent header — this component's own
   *  nav would just duplicate it, so it's off by default here. */
  showNav?: boolean;
  dotColors?: string[];
  dotBgColor?: string;
};
...
export function DotmatrixHero({
  badge = "Enterprise AI",
  titleLine1 = "AI Infrastructure Built",
  titleLine2 = "for Enterprise Scale.",
  description = "Deploy secure, production-ready AI systems that unify your data, automate workflows, and accelerate decision-making across every team.",
  ...
  joinLabel = "Join Us",
  showNav = true,
  dotColors = ["#01050f", "#052054", "#0b43a2"],
  dotBgColor = "#000000",
}: DotmatrixHeroProps = {}) {
  return (
    <section className="ok-h26-hero">
      {showNav && (
        <nav className="ok-h26-nav">
          <div className="ok-h26-logo">
            <img
              src={asset("logomark.svg")}
              alt="Logo"
              className="ok-h26-logomark"
            />
          </div>
          <div className="ok-h26-nav-links">
            ...
            <a className="ok-h26-nav-pill ok-h26-nav-pill--cta" href="#">
        ...
        <div className="ok-h26-hero-inner">
          <div className="ok-h26-hero-content">
            ...
            <div className="ok-h26-cta-row">
              <a className="ok-h26-btn ok-h26-btn--white" href={primaryCtaHref}>
                <span className="ok-h26-btn__label">{primaryCtaLabel}</span>
                <span className="ok-h26-btn__icon">
                  <img src={asset("arrow-right-1.svg")} alt="" />
                </span>
              </a>
              <a className="ok-h26-btn ok-h26-btn--blue" href={secondaryCtaHref}>
                <span className="ok-h26-btn__label">{secondaryCtaLabel}</span>
                <span className="ok-h26-btn__icon">
                  <img src={asset("arrow-right-2.svg")} alt="" />
                </span>
              </a>
            </div>
          </div>
