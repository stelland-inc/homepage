...
// Colors a tube's vertices by latitude (derived straight from each vertex's
// own y-position on the sphere) so a single line can blend from `colorFrom`
// at the north pole to `colorTo` at the south pole, independent of how
// TubeGeometry resamples the curve internally.
function applyLatitudeGradient(
    ...
    for (let i = 0; i < posAttr.count; i++) {
        ...
        const t = (90 - latDeg) / 180;
        blended.copy(colorFrom).lerp(colorTo, t);
...
interface DotsConfig {
    ...
    density: number;
    allDots: boolean;
    // Optional: when set, dots blend from `color` (north pole) to this
    // color (south pole) by latitude, instead of one flat color.
    gradientTo?: string;
}
interface GlobeProps {
    ...
    // Optional: when set, grid lines blend from `graticuleColor` (north
    // pole) to this color (south pole) by latitude, instead of one flat
    // color — mirrors dots.gradientTo.
    graticuleColorTo?: string;
    showGrid?: boolean;
    outlineWidth?: number;
    dragSpeed?: number;
    detail?: number;
...
export default function Globe({
    ...
    useEffect(() => {
        ...
        // Built from the already-parsed rgba (not `new Color(resolvedOceanColor)`
        // directly) because THREE.Color can't parse 8-digit hex-with-alpha
        // strings like "#FF81971a" — it silently no-ops and logs a warning,
        // which previously left these colors wrong.
        const oceanColorObj = resolvedOceanColor
            ? new Color(oceanRgba.r, oceanRgba.g, oceanRgba.b)
            : new Color(0, 0, 0);
        ...
        if (showGrid && resolvedGraticuleColor && graticuleRgba.a > 0) {
            const graticuleColorObj = resolvedGraticuleColor
                ? new Color(graticuleRgba.r, graticuleRgba.g, graticuleRgba.b)
                : new Color(1, 1, 1);
            const graticuleGradientToRgba = graticuleColorTo
                ? parseColorToRgba(graticuleColorTo)
                : null;
            const graticuleGradientToColor = graticuleGradientToRgba
                ? new Color(
                      graticuleGradientToRgba.r,
                      graticuleGradientToRgba.g,
                      graticuleGradientToRgba.b
                  )
                : null;
            const graticuleMaterial = new MeshBasicMaterial({
                color: graticuleGradientToColor ? 0xffffff : graticuleColorObj,
                vertexColors: !!graticuleGradientToColor,
            ...
            try {
                ...
                if (fill === "solid") {
                    ...
                    if (dotCoordinates.length > 0) {
                        ...
                        const gradientToColor = dots.gradientTo
                            ? new Color(dots.gradientTo)
                            : null;
                        const dotMaterial = new MeshBasicMaterial({
                            color: gradientToColor ? 0xffffff : dotColorObj,
                            vertexColors: !!gradientToColor,
                        ...
                        for (let i = 0; i < dotCoordinates.length; i++) {
                            ...
                            if (gradientToColor) {
                                // North pole (lat 90) stays the base color,
                                // south pole (lat -90) blends fully into
                                // gradientTo — a fixed navy-to-pink sweep
                                // that rotates with the globe.
                                const t = (90 - lat) / 180;
                                blended.copy(dotColorObj).lerp(gradientToColor, t);
                                instanced.setColorAt(i, blended);
                            }
                        }
